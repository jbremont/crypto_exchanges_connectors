...
class Api(object):
    ...
    def get_balance(self, currency=None):
        ...
        post_parameters = {'Currency': currency} if currency is not None else {}

        result, error = self.api_query(feature_requested='GetBalance',
                                       post_parameters=post_parameters)

        if error is None:
            result = result[0] if currency is not None else result
        return result, error
    ...
    def get_tradehistory(self, market, count):
        ...
        return self.api_query(feature_requested='GetTradeHistory',
                              post_parameters={'Market': market, 'Count': count})
